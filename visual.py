...
class Organism(Cell):
    ...
    def draw(self):
        ...
        pygame.draw.circle(GUI.SCREEN, self.color, (x, y), Organism.radius)
...
class Plant(Cell):
    ...
    def draw(self):
        x = self.x * COEFF + COEFF // 2
        y = self.y * COEFF + COEFF // 2
        pygame.draw.circle(GUI.SCREEN, Plant.color, (x, y), Plant.radius, 4)
...
class Wall(Cell):
    ...
    def draw(self):
        x = self.x * COEFF
        y = self.y * COEFF
        pygame.draw.rect(GUI.SCREEN, Wall.color, (x, y, Wall.size[0], Wall.size[1]))
...
class GUI:
                            ...
                            for x, y in self.queue_cell.pop():
                                ...
                                if self.environment.get_cell(x, y).cell_type != "empty":
                                    self.environment.set_cell(
                                        x,
                                        y,
                                        Cell(x, y, "empty", self.environment.light),
                                    )
                    ...
                    for i in range(-1, 1):
                        ...
                        for j in range(-1, 1):
                            ...
                            self.add_cell(
                                x_c,
                                y_c,
                                Wall(x_c, y_c, self.environment.light),
                                [16, i, j],
                            )
    ...
    def erase_cell(self):
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                x, y = x // self.coeff, y // self.coeff
                if (
                    x < GUI.DISPLAY_X - GUI.MENU_SIZE
                    and y < GUI.DISPLAY_Y
                    and self.environment.get_cell(x, y).cell_type != "empty"
                ):
                    self.environment.set_cell(
                        x, y, Cell(x, y, "empty", self.environment.light)
                    )
    ...
    def add_cell(self, x, y, cell, size=None):
        ...
        if cell.cell_type == "wall":
            if (
                x * self.coeff + self.coeff < GUI.DISPLAY_X - GUI.MENU_SIZE
                or y * self.coeff <= GUI.DISPLAY_Y
            ):
            ...
            if (
                x * self.coeff < GUI.DISPLAY_X - GUI.MENU_SIZE
                or y * self.coeff < GUI.DISPLAY_Y
            ):
