...
class Automata:
    ...
    def _abilities_decider(self) -> None:
        abilities_namings = [self.see_ability, self.move_ability, self.kill_ability, self.eat_ability,
                             self.photosynth_ability, self.hybernate_ability, self.reproduce_ability,
    ...
    def move_ability(self, strength) -> None:
        ...
        def get_all_ranges(possible_moves,
                           track_list):  # розраховує відстань до кожного об'єкту зі списку track_list, яка буде між автоматом і об'єктом якщо він переміститься на якийсь з можливих селлів. Формат аутпуту функції : "x_possible_to_go y_possible_to_go" : [(Cell(x_possible_to_go, y_possible_to_go), range_to_an object1), ...]
            ranges = []
            for i in track_list:
        ...
        def escape(
                danger_cells):  # визначає найкращі за векторним добутком координати для ВТЕЧІ і повертає їх у формі ("x", "y")
        ...
        def move_towards(
                pray_cells):  # визначає найкращі за векторним добутком координати для ПОГОНІ і повертає їх у формі ("x", "y")
            possible_moves = get_possible_moves()
        ...
        def look_for_pray():
            return [x for x in look_for_danger() if \
                    x.organism._abilities_decider()[x.organism.kill_ability] <= self._abilities_decider()[
                        self.kill_ability]]
    ...
    def eat_ability(self,
                    strength) -> None:  # їсть якщо стоїть на клітинці з їжею або їжа є на сусідніх клітинках. cell_type клітинки змінюється на "empty" і автомату додається енергія.
    ...
    def photosynth_ability(self, strength) -> None:
        self.energy += floor(strength * self.cell.light / 2) if self.energy + floor(
            strength * 1.5) <= 50 else 50 - self.energy
        return False
    ...
    def mutate(self) -> None:
        ...
        self.genome = self.genome[:mutation_position] + str(int(self.genome[mutation_position]) ^ 1) + self.genome[
                                                                                                       mutation_position + 1:]
    ...
    def crossover(self, other) -> None:
        ...
        self.genome, other.genome = self.genome[0:crossing] + other.genome[crossing:], other.genome[
                                                                                       0:crossing] + self.genome[
                                                                                                     crossing:]
        return self, other
